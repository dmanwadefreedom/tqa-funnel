# TQA APRIL 11 BATTLE PLAN
## 5 Days Out. Everything Below Must Happen.

---

## FOR JAY (Closer) — DO TODAY

Call these 6 applicants. They applied 18-21 days ago, scored HOT, attended the webinar, and got ghosted. Be human about it.

| Name | Phone | Email | Applied |
|------|-------|-------|---------|
| Kenny Ganta | +12033938088 | kenny.ganta@gmail.com | Mar 16 |
| Shafiul Kh | +19049161990 | shafiulkh360@gmail.com | Mar 17 |
| Muhammad Mazed | +13478497564 | majed.assurance@gmail.com | Mar 18 |
| Rama Krishna Guttula | +12106872552 | grkrc777@gmail.com | Mar 18 |
| Ejaz Khan | +15162442307 | e.khan1@yahoo.com | Mar 19 |
| Sonny Pierre | +15619168802 | sjdfinancial@gmail.com | Mar 19 |

**Script:**
"Hey [name], this is Jay calling from The Quantum Achievers. You applied to DJ's program a few weeks back and I owe you an apology. We dropped the ball on getting back to you. If you're still thinking about the move into cybersecurity, DJ's running a live career briefing on April 11. I'd love to get you a seat. Are you still interested?"

If they say they can't afford it: "I hear you. The briefing itself is just $47 for 90 minutes with DJ. No commitment beyond that. It's designed to give you the full picture before you decide anything."

Do NOT hard sell. These people already said yes once and got ignored. Rebuild trust first.

---

## FOR SHREYAS (Ads) — DO TODAY/TOMORROW

### Step 1: Upload Custom Audience
File ready: `~/Documents/TQA/Client Files/tqa-contacts-meta-upload.csv`
- 197 contacts (email + phone + name)
- Upload to Meta Ads Manager > Audiences > Customer List
- Name it: "TQA Webinar Registrants March 2026"

### Step 2: Create Lookalike
- 1% Lookalike off the custom audience
- Target: United States
- Name it: "TQA 1% Lookalike - Webinar Regs"

### Step 3: Build Cold Audience
- Age: 28-42
- Location: Dallas-Fort Worth, SF Bay Area, Seattle, Chicago, NJ/NYC, Atlanta, Houston, Raleigh-Durham, Washington DC
- Interests: Cybersecurity, CISSP, CompTIA, Information Security, IT Certification
- Cultural layer: TCS, Infosys, Wipro, Bollywood, Times of India
- EXCLUDE: Custom audience (don't retarget people already in the list)
- Name it: "TQA Cold - IT Pros Cyber Interest"

### Step 4: Budget
- Retarget (Audience A): AUD $15/day
- Lookalike (Audience B): AUD $20/day
- Cold (Audience C): AUD $15/day
- Total: AUD $50/day = ~$250 for 5 days before April 11

### Step 5: Ad Creative
Landing page: https://dmanwadefreedom.github.io/tqa-jumpstart/masterclass.html
(Updated with fresh 2026 stats, $47 pricing)

---

## FOR AGRESH (GHL) — DO TODAY

### Critical Fixes
1. Settings > Business Info > Timezone: Change "America/Detroit" to "America/New_York"
2. Settings > Business Info > Email: Change "extromarketingagency@gmail.com" to "achieversquantum@gmail.com"
3. Assign 6 applicants (listed above) to Jay
4. Move 6 applicants to "Applied" stage in Webinar pipeline

### Workflow Updates (ALL of these before April 11)
5. **Webinar Confirmation** workflow: Update all dates from March 19 to April 11, new Zoom link
6. **Webinar Reminders** workflow: Update March 19 to April 11 (24h, 1h, 15min reminders)
7. **PUBLISH** these draft workflows:
   - New Lead Nurture (Fast 5) — CRITICAL
   - Meta Lead Registration — CRITICAL (or new ad leads get nothing)
   - Long-Term Nurture
   - Appointment Confirmation
   - Post Call Payment Follow Up

### Email Sequence for April 11 (5-day countdown)
Build these as a new workflow triggered by tag: `tqa-april11-funnel`

**Day 1 (April 6):** "DJ here. April 11 is 5 days away."
**Day 2 (April 7):** Share one educational insight (certification path or salary data)
**Day 3 (April 8):** Short personal video from DJ (30-60 sec, "here's what we'll cover")
**Day 4 (April 9):** Social proof / Cyber Caffeine podcast episode link
**Day 5 (April 10):** Final reminder + logistics (Zoom link, time, what to have ready)
**Day of (April 11, 9 AM EST):** "We start in 2 hours. Here's your link."
**Day of (April 11, 10:45 AM EST):** "15 minutes. See you inside."

---

## FOR DJ — DO THIS WEEK

### Trust Content (creates before April 11)
1. **30-60 second personal video greeting** — "Hey, I'm DJ. Here's what to expect on April 11." Film on phone, upload unlisted to YouTube. Agresh puts in confirmation email.
2. **Share a Cyber Caffeine episode** relevant to career transitions. Link in Day 4 email.
3. **One LinkedIn post** this week about the current threat landscape (use the fresh stats: 514K unfilled, 86% breached, AI 4x faster). End with briefing mention.

### Sales Navigator
4. **Reactivate Sales Navigator** ($99/mo). The entire LinkedIn DM funnel depends on it.
5. Once active, save the search "TQA - IT Pros Ready for Cyber" with the filters above.
6. Start sending 20-25 connection requests/day (Tuesday-Thursday best).

### Socials Check
7. Update ALL bios to match the unified bio from package.html Section 3.
8. Platforms: LinkedIn, YouTube, Facebook, Twitter/X, Telegram, Spotify (Cyber Caffeine)

---

## RE-ENGAGEMENT EMAIL (for 6 applicants)

Subject: DJ owes you an apology

Body:
```
[First name],

I'll be straight with you. You applied to The Quantum Achievers a few weeks ago, and we didn't get back to you fast enough. That's on us.

If you're still thinking about the move into cybersecurity, I want you to know the door is open.

Here's what's happening: I'm running a live Career Briefing on April 11 at 11 AM EST. 90 minutes. I'll walk through the exact career path, the certification roadmap that matters, salary data at every level, and how the hidden job market works. I'll also do a live LinkedIn audit for one attendee.

You already took the step of applying. That tells me you're serious. I'd like to get you in the room.

$47 holds your seat. If that's a concern right now, reply to this email and let's talk.

Link: [BRIEFING_LINK]

DJ
```

This handles the "can't afford" crowd without being a pushover. $47 is the filter. If they can't do $47, they're not buying a $4,997 program.

---

## SEGMENT STRATEGY (all 197 contacts)

| Segment | Tag | Count | Action | Email Angle |
|---------|-----|-------|--------|-------------|
| Applicants (HOT) | tqa-applied | 6 | Jay calls + re-engage email above | "We owe you an apology" |
| Hot no-shows | tqa-reactivate-hot-noshow | 54 | Email sequence | "You registered. Life happened. April 11 is your shot." |
| Attended webinar | webinar-attended | 12 | Email + personal touch | "You were in the room. Here's what changed since March." |
| Warm leads | tqa-reactivate-warm | 7 | Email sequence | "Still thinking about cybersecurity?" |
| Old/stale | old-lead + cleanup | 51 | Long-term nurture | Weekly value email, no hard sell |
| Cold/unscored | remaining | ~67 | Broad nurture | Educational content only |

---

## PRE-QUALIFICATIONS CHECKLIST

Before April 11, verify:
- [ ] Zoom link works and is in all workflows
- [ ] Confirmation email sends immediately on registration
- [ ] SMS reminders: 24h, 1h, 15 min before
- [ ] Timezone displayed correctly (11 AM EST / 8 AM PST / 11 PM IST)
- [ ] Stripe payment link works for $47
- [ ] Post-webinar follow-up workflow is PUBLISHED
- [ ] Jay has his closer script with current pricing
- [ ] DJ has the slide deck loaded (briefing-slides.html)
- [ ] Recording: there IS no recording (that's the point, verify it's not auto-recording)
- [ ] LinkedIn audit exercise: DJ has a volunteer or will pick from attendees

---

## FILES READY

| File | Location | Purpose |
|------|----------|---------|
| Contact CSV for Meta | ~/Documents/TQA/Client Files/tqa-contacts-meta-upload.csv | Shreyas uploads to Meta |
| Updated package page | dmanwadefreedom.github.io/tqa-jumpstart/package.html | Internal reference |
| Updated masterclass page | dmanwadefreedom.github.io/tqa-jumpstart/masterclass.html | Briefing registration |
| Updated accelerator page | dmanwadefreedom.github.io/tqa-jumpstart/accelerator.html | Program offer |
| Atlas agent | ~/.openclaw/agents/atlas/agent/system-prompt.md | Growth specialist (wire later) |
| This battle plan | ~/Documents/TQA/Client Files/APRIL-11-BATTLE-PLAN.md | Master checklist |
